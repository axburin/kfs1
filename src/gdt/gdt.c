...
// * Tss is commented because not needed in kfs2
...
void initGdt(){
    ...
    setGdtGate(2, 0, 0xFFFFFFFF, 0x92, 0xCF); // data 
    setGdtGate(3, 0, 0xFFFFFFFF, (uint8_t)GDT_STACK_PL0, 0xCF); // kernel stack
    setGdtGate(4, 0, 0xFFFFFFFF, 0xFA, 0xCF); // code user  
    setGdtGate(5, 0, 0xFFFFFFFF, 0xF2, 0xCF); // data user
    setGdtGate(6, 0, 0xFFFFFFFF, (uint8_t)GDT_STACK_PL3, 0xCF); // user stack
    // writeTSS(5, 0x10, 0x0);
